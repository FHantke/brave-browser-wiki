...
- Sometimes strings have variables in them which should look like this, in these cases you should use < and > and not `&lt;` and not `&gt;`.  These are `ph` and `em` elements.  Even though the source looks like `&lt;ph` the translation  should use `<` and `>`.<br/> In the example below the translation should look like this:
`<ph name="EXTENSION_NAME">$1</ph> (translate this "<ph name="EXTENSION_ID">$2<ex>do not translate this</ex></ph>") translate this too.`<br/>
Note, that `ex` element should also use `<` and `>`, the text inside `ex` should not be translated - it's an example of values that would be used in this variable.
