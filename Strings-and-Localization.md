...
- `_override.grd` files are generated automatically (via `npm run chromium_rebase_l10n`) and should not be updated. There are a [series of regex replacements done](https://github.com/brave/brave-core/blob/master/script/lib/grd_string_replacements.py) (ex: Chrome => Brave) and these `_override.grd` files will get overwritten each time.
