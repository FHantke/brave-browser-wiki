...
- If something is wanted in Release channel, then a PR must be made to the current [version branch which is in Release](https://github.com/brave/brave-browser/wiki/Brave-Release-Schedule#current-channel-information). An approval (Via reviewer) is required by someone on the [uplift approvers list](https://github.com/brave/brave-browser/wiki/Triage-Guidelines#uplift-approvers) below.   
